...
export function generateFractionSimplify(difficulty=2){
 ...
 return createQuestion({id:`fraction-${shared}-${num}-${den}`,partId:'B',chapterId:'rational',notionId:'fraction-simplify',difficulty,questionHtml:`Déterminer l’ensemble de définition, puis simplifier <span class="math">${n}/${d}</span>.`,hint:'Commencer par les zéros du dénominateur initial, puis simplifier les facteurs communs.',correctionHtml:`Domaine : <span class="math">ℝ \\ {${shared}; ${den}}</span>. Pour <span class="math">x ≠ ${shared}</span>, la fraction se simplifie en <span class="math">${factor(num)}/${factor(den)}</span>.`,hiddenConcept:'La valeur interdite issue du facteur simplifié reste interdite dans la fonction initiale.',oralFormulation:'« Je détermine le domaine avant toute simplification et je conserve les exclusions initiales. »',generator:'fractions.simplify'});
}
