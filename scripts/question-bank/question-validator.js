...
export function validateQuestion(q,{publishing=q.status==='published'}={}){
 ...
 if(![1,2,3,4].includes(Number(q.difficulty)))errors.push('Difficulté invalide');
 ...
 if(!['course','formula','parameterized'].includes(q.question_type))errors.push('Type invalide');
 if(q.question_type==='course'&&Number(q.difficulty)===3)errors.push('Le niveau « Réflexe prépa » est réservé aux calculs et aux formules.');
 if(publishing&&['formula'].includes(q.question_type)&&!hasMath(q))errors.push('Une question de formule doit contenir une expression mathématique.');
 if(publishing&&Number(q.difficulty)===3&&!hasMath(q))errors.push('Une question « Réflexe prépa » doit contenir un calcul ou une formule.');
 if(q.question_type!=='parameterized'&&q.variable_spec!=null)errors.push('Variables réservées au type paramétré');
 if(q.question_type==='parameterized'){
