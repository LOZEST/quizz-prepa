...
export function generatePowerProduct(difficulty=1){
 ...
 return createQuestion({id:`pow-product-${base}-${m}-${n}`,fingerprint:`pow-product-${m}-${n}`,partId:'A',chapterId:'powers',notionId:r<0?'powers-negative':'powers-product',difficulty,questionHtml:`Simplifier <span class="math">${power(base,m)} \\times ${power(base,n)}</span>. Donner d’abord une écriture avec un seul exposant, puis sans exposant négatif. Préciser les conditions.`,hint:'Pour un produit de puissances de même base, on additionne les exposants.',correctionHtml:r>=0?`<span class="math">${power(base,m)} \\times ${power(base,n)} = ${power(base,r)}</span>. Condition : ${condition}.`:`<span class="math">${power(base,m)} \\times ${power(base,n)} = ${power(base,r)} = \\dfrac{1}{${power(base,-r)}}</span>. Condition : ${condition}.`,hiddenConcept:'Les règles sur les exposants ne dispensent pas de vérifier les valeurs interdites.',oralFormulation:'« Les puissances ont la même base, donc j’additionne les exposants. »',generator:'powers.product'});
}
