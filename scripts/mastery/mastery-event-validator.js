...
const RESULTS=new Set(['success','partial','failed','easy','hard','fragile']);
...
export function validateMasteryEvent(event,{notionIds,now=Date.now(),futureToleranceMs=300000}={}){
 ...
 if(['easy','success'].includes(e.selfEvaluation)&&e.hintUsed)errors.push('réussite complète interdite avec indice');if(!finite(e.scoreRatio)||e.scoreRatio<0||e.scoreRatio>1)errors.push('scoreRatio invalide');if(!finite(e.pointsObtained)||!finite(e.pointsMaximum))errors.push('points invalides');if(e.gradingMode!=null&&!['self','teacher','other'].includes(e.gradingMode))errors.push('correction invalide');
