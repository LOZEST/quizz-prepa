...
export function buildAdaptivePlan({state,events=[],now=new Date(),days=14,maxPerDay=4}={}){
 const current=new Date(now),today=dateOnly(current),overview=accountOverview(state,current.getTime()),notions=new Map(overview.notions.map(n=>[n.id,n])),chapters=new Map(chapterOptions().map(c=>[c.id,c])),normalized=normalizeEvents(events),slots=Array.from({length:days},(_,index)=>({date:dayKey(new Date(today.getTime()+index*DAY)),tasks:[]}));
 ...
 return{generatedAt:current.toISOString(),overview,events:normalized,days:slots};
}
