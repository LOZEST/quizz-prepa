...
test('le mode test utilise des dialogues internes, expose les erreurs et ne contient aucun confirm natif',async()=>{const[js,html,css]=await Promise.all(['../scripts/tests/ui/test-app.js','../quiz.html','../styles/app.css'].map(path=>readFile(new URL(path,import.meta.url),'utf8')));assert.doesNotMatch(js,/\bconfirm\s*\(/);assert.match(html,/data-test-dialog/);assert.match(html,/Continuer le test/);assert.match(js,/Rendre définitivement/);assert.match(js,/Test abandonné/);assert.match(html,/role="alert"/);assert.match(css,/body\.test-active\{position:fixed/);for(const action of["'prev'","'next'","'jump'","'submit'","'abandon'"])assert.ok(js.includes(action),`action absente ${action}`);assert.match(js,/automatic:true/)});
