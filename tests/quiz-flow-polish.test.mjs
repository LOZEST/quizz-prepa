...
test('Passer advances immediately and undo redo expose reliable state',async()=>{const source=await read('scripts/app.js');assert.match(source,/function\s+skipQuestion\s*\(\s*\)\s*\{[\s\S]*?evaluate\(RESULT\.SKIPPED\);[\s\S]*?newQuestion\(\);/);assert.match(source,/undoStack\.length\s*===\s*0/);assert.match(source,/redoStack\.length\s*===\s*0/);assert.match(source,/event\.shiftKey\s*\?\s*'redo'\s*:\s*'undo'/)});
